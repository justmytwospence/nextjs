...
import type { DetailedActivity, DetailedSegment, Route, SummaryActivity } from "@/lib/strava/schemas/strava";
import { DetailedActivitySchema, DetailedSegmentSchema, RoutesSchema, SummaryActivitiesSchema } from "@/lib/strava/schemas/strava";
...
export const fetchRoutes = withRetry()(async (
  userId: string,
  perPage: number = 10,
  page: number = 1
): Promise<Route[]> => {
...
export const fetchDetailedSegment = withRetry()(async (
  userId: string,
  segmentId: string
...
export const fetchRouteGeoJson = withRetry()(async (
  userId: string,
  routeId: string
): Promise<JSON> => {
...
export const fetchActivities = withRetry()(async (
  userId: string,
  perPage: number = 10,
  page: number = 1
...
export const fetchDetailedActivity = withRetry()(async (
  userId: string,
  activityId: string
