...
import SyncSegmentsButton from "@/components/sync-segments-button";
...
export default function CoursesPage() {
  ...
  const initialSortBy =
    (searchParams.get("sortBy") as SortOption) || "createdAt";
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => fetchCourses(selectedTab, selectedType, currentPage),
  ...
  useEffect(() => {
    if (currentPage < totalPages) {
      queryClient.prefetchQuery({
        ...
        queryFn: () => fetchCourses(selectedTab, selectedType, currentPage + 1),
        staleTime: Number.POSITIVE_INFINITY,
        gcTime: Number.POSITIVE_INFINITY,
      });
    }
