...
"$CONDA_DIR"/bin/conda run -n gdal_env cmake .. \
  ...
  -DCMAKE_CXX_FLAGS="--sysroot=$CONDA_DIR/envs/gdal_env -I$CONDA_DIR/envs/gdal_env/include -L$CONDA_DIR/envs/gdal_env/lib -Wl,--dynamic-linker=/var/task/dylibs/ld-linux-x86-64.so.2" \
  -DCMAKE_C_FLAGS="--sysroot=$CONDA_DIR/envs/gdal_env -I$CONDA_DIR/envs/gdal_env/include -L$CONDA_DIR/envs/gdal_env/lib -Wl,--dynamic-linker=/var/task/dylibs/ld-linux-x86-64.so.2" \
  -DCMAKE_INSTALL_PREFIX="$CONDA_DIR"/envs/gdal_env \
  -DCMAKE_SHARED_LINKER_FLAGS="--sysroot=$CONDA_DIR/envs/gdal_env -L$CONDA_DIR/envs/gdal_env/lib -Wl,-rpath,$CONDA_DIR/envs/gdal_env/lib" \
