...
mkdir build
cd build
$HOME/miniconda3/bin/conda run -n gdal_env cmake .. -BUILD_APPS=OFF -BUILD_PYTHON_BINDINGS:BOOL=OFF -BUILD_SHARED_LIBS=OFF -DCMAKE_ARCHIVE_OUTPUT_DIRECTORY=. -DCMAKE_BUILD_TYPE=Release -DGDAL_BUILD_OPTIONAL_DRIVERS=OFF -DGDAL_ENABLE_DRIVER_GTIFF=ON -DGDAL_ENABLE_DRIVER_MEM=ON -DOGR_BUILD_OPTIONAL_DRIVERS=OFF
echo "Building GDAL"
